...
router.post('/register', async (req, res) => {
  try {
    ...
    const user = new User({
      ...
      isApproved: false // По умолчанию ждет подтверждения
    });
...
router.post('/login', async (req, res) => {
  try {
    ...
    // ПРОВЕРКА: одобрен ли аккаунт
    if (!user.isApproved) {
      return res.status(403).json({ error: 'Ваш аккаунт еще не подтвержден администратором' });
    }
